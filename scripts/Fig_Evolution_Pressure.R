############################################################
# Fig_Evolution_Pressure.R
#
# Three Evolution Pressure panels (from Evolution_pressure_code):
...
#
# Built for continuous (Sleep duration + NREM ratio) and
# binary (Sleep timing + Sleep frequency) phenotype groups.
#
...
#
# Output:
#   Result/Evolution_Pressure/*.jpg|.pdf
#
...
GRP <- list(
  cont = c("Sleep_duration", "NREM_ratio"),
  bin  = c("Sleep_timing", "Sleep_frequency")
)
PCOL <- c(
...
## =========================================================
## 4. Build P1 / P2 / P7 for each phenotype group
## =========================================================
...
mk <- function(grp) {
  ...
  p1 <- ggplot(d, aes(omega_M0, Phenotype, fill = Phenotype)) +
    ...
    labs(title = "Gene-wide dN/dS (M0)",
         x = "ω  (dN/dS)", y = NULL,
         subtitle = paste0("diamonds = highlighted genes; ", nlab)) +
    th
  save_fig(p1, paste0(tagp, "_P1_omega_distribution"), width = 7, height = 5)

  ## ── P2. Constraint vs site-class selection ───────────────────────
  ...
  p2 <- ggplot(d2, aes(omega_M0, y)) +
    ...
    annotate("text", x = 1.02, y = 8, hjust = 0, vjust = 0,
             ...
             label = "ω = 1") +
    labs(
      title = "Gene-wide constraint versus site-class positive selection",
      subtitle = paste0(
        "Above the dashed line: M1a vs M2a FDR < 0.05. ",
  ...
  sub7 <- if (has_m7) {
    "FDR < 0.05; M7/M8 incomplete and prone to false positives"
  } else {
    "FDR < 0.05 (M1a vs M2a); M7/M8 not present in input tables"
  }
  ...
  p7 <- ggplot(b, aes(Phenotype, pct, fill = test)) +
    ...
    labs(title = "Genes with site-level positive selection",
...
message("Done. Outputs under Result/Evolution_Pressure/")
